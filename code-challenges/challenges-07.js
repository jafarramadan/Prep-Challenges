...
const objLat = (obj) => {
    const cfirstName=obj.firstName.charAt(0).toUpperCase()+obj.firstName.slice(1);
    const clastName=obj.lastName.charAt(0).toUpperCase()+obj.lastName.slice(1);

  return `my name is ${cfirstName} ${clastName} I am ${obj.age} YO, and I love ${obj.hobby}.`;
...
let cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        let notNull = arr[i];
        if (notNull.yearsOfExperience > 1) {
            let fullName = '';
            if (notNull.firstName !== null) {
                fullName += notNull.firstName;
            }
            if (notNull.lastName !== null) {
                fullName += (fullName ? ' ' : '') + notNull.lastName;
            }
            resultCVs.push({
                fullName,
                tech: notNull.tech
